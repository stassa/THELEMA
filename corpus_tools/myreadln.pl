...
readln(Stream, Read) :-			% the default is read up to EOL
	...
	(   (LastCh == -1; LastCh == end_of_file )
	->  append(Line,[end_of_file], Read)
	;   Read = Line
	).

readln(Stream, Read, LastCh):-
	string_codes("_0123456789", Arg2),
	rl_readln(Stream, Read, LastCh, [10], Arg2, uppercase).
...
readln(Stream, P, [EOF], StopChars, WordChars, Case) :-
	...
	rl_readln(Stream, P, EOF, Arg1, Arg2, Arg3).

rl_readln(Stream, P, EOF, StopChars, WordChars, Case) :-
	rl_initread(Stream, L, EOF, StopChars),
	rl_blanks(L, LL), !,
	rl_words(P, LL,[], options(WordChars, Case)), !.

rl_initread(Stream, S, EOF, StopChars) :-
	get_code(Stream,K),
	rl_readrest(Stream, K, S, EOF, StopChars).

rl_readrest(_,-1, [], end_of_file, _) :- !.
rl_readrest(Stream,0'\\, [K1|R], EOF, StopChars) :-
	get_code(Stream,K1),			% skip it, take next char
	get_code(Stream,K2),
	rl_readrest(Stream, K2, R, EOF, StopChars).
rl_readrest(_,K, [K], K, StopChars) :-	% the stop char(s)
	member(K, StopChars), !.
rl_readrest(Stream,K, [K|R], EOF, StopChars) :-	% the normal case
	get_code(Stream,K1),
	rl_readrest(Stream, K1, R, EOF, StopChars).
